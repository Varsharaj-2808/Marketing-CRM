  ...
  try {
    ...
    try {
      const updatedLeads = await Promise.all(
        leadsToProcess.map(l => Lead.findById(l.id))
      );
      for (const lead of updatedLeads) {
        if (lead) {
          await algolia.saveLead(lead);
        }
      }
      console.log(`[bulkAssign] Indexed ${updatedLeads.length} leads to Algolia`);
    } catch (algoliaErr) {
      console.error('[bulkAssign] Algolia indexing error:', algoliaErr.message);
    }
