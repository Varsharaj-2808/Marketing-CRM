...
exports.createLead = async (req, res, next) => {
  try {
    ...
    await algolia.saveLead(lead).catch(err => console.error('[createLead] Algolia indexing skipped:', err.message));
...
exports.updateLeadStage = async (req, res, next) => {
  try {
    ...
    await algolia.saveLead(updatedLead).catch(err => console.error('[updateLeadStage] Algolia indexing skipped:', err.message));
...
exports.closeLeadLost = async (req, res, next) => {
  try {
    ...
    await algolia.saveLead(updatedLead).catch(err => console.error('[closeLeadLost] Algolia indexing skipped:', err.message));
...
exports.closeLeadWon = async (req, res, next) => {
  try {
    ...
    await algolia.saveLead(updatedLead).catch(err => console.error('[closeLeadWon] Algolia indexing skipped:', err.message));
...
exports.searchLeads = async (req, res, next) => {
  try {
    const {
      q, stage, priority, lead_source, category, sub_category,
      assigned_to, status, city,
      page, limit,
    } = req.query;
    ...
    const result = await algolia.searchLeads(
      q || '',
      filters,
      parseInt(page) || 1,
      parseInt(limit) || 20
    ...
    if (!result) {
      return res.status(502).json({ success: false, message: 'Search service unavailable' });
    }
...
exports.reindexLeads = async (req, res, next) => {
  try {
    const result = await query('SELECT * FROM leads WHERE deleted_at IS NULL ORDER BY created_at DESC');
    await algolia.indexAllLeads(result.rows);
    res.json({ success: true, message: `Re-indexed ${result.rows.length} leads to Algolia.` });
  } catch (error) {
    next(error);
  }
};
...
exports.closeLead = async (req, res, next) => {
  try {
    ...
    await algolia.saveLead(updatedLead).catch(err => console.error('[closeLead] Algolia indexing skipped:', err.message));
