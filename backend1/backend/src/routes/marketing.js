...
router.get('/algolia/configure', protect, authorize('Admin', 'Marketing Executive'), async (req, res, next) => {
  try {
...
router.get('/leads/search', protect, authorize('Admin', 'Marketing Executive'), leadController.searchLeads);
router.get('/leads/reindex', protect, authorize('Admin', 'Marketing Executive'), leadController.reindexLeads);
