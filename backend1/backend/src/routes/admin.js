...
router.get('/users/search', protect, authorize('Admin'), userController.searchUsers);
...
router.get('/algolia/configure', protect, authorize('Admin'), async (req, res, next) => {
  try {
    await algolia.configureIndices();
    res.json({ success: true, message: 'Algolia indices configured successfully.' });
  } catch (error) {
    next(error);
  }
});
...
router.get('/leads/search', protect, authorize('Admin'), leadController.searchLeads);
router.get('/leads/reindex', protect, authorize('Admin'), leadController.reindexLeads);
