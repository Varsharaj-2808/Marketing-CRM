...
export default function BulkAssignModal({ isOpen, onClose, selectedCount, selectedLeads, onAssign, assigning }) {
  ...
  const anyHasOwner = selectedLeads?.some(
    (lead) => !!(lead.assignedTo ?? lead.assigned_to)
  );
  ...
  return (
    <Modal isOpen={isOpen} onClose={handleClose} title={`Assign ${selectedCount} Lead${selectedCount > 1 ? 's' : ''}`}>
      <p className="mb-4 font-body-md text-body-md text-on-surface-variant">
        {selectedCount} lead{selectedCount > 1 ? 's' : ''} selected.
        {anyHasOwner
          ? ' Some leads already have an owner. Provide a reason for the reassignment.'
          : ' Select a Marketing Executive to assign these leads to.'}
      </p>
      ...
      <div className="mt-6 flex justify-end gap-3">
        ...
        >
          ...
          {assigning ? 'Assigning...' : anyHasOwner ? 'Reassign' : 'Assign'}
        </button>
      </div>
    </Modal>
  );
