...
exports.getAuditRetention = async (req, res, next) => {
  try {
    ...
    if (!result.rows[0]) {
      return res.json({ success: true, message: 'Audit retention fetched successfully', data: { key: 'audit_log_retention_months', value: '12' } });
    }
    res.json({ success: true, message: 'Audit retention fetched successfully', data: result.rows[0] });
  } catch (error) {
    next(error);
  }
};
