...
exports.checkMobile = async (req, res, next) => {
  try {
    ...
    if (existing) {
      return res.json({ success: true, message: 'Duplicate mobile found', data: { isDuplicate: true, leadId: existing.lead_id } });
    }
    res.json({ success: true, message: 'Mobile is unique', data: { isDuplicate: false } });
  } catch (error) {
    next(error);
  }
};
...
exports.checkEmail = async (req, res, next) => {
  try {
    ...
    if (existing) {
      return res.json({ success: true, message: 'Duplicate email found', data: { isDuplicate: true, leadId: existing.lead_id } });
    }
    res.json({ success: true, message: 'Email is unique', data: { isDuplicate: false } });
  } catch (error) {
    next(error);
  }
};
...
exports.getAdminLeads = async (req, res, next) => {
  try {
    ...
    res.json({
      success: true,
      message: 'Leads fetched successfully',
      data: { page: result.page, totalPages: result.totalPages, totalCount: result.totalCount, limit: result.limit || limitNum, data: result.data },
    });
  } catch (error) {
    next(error);
  }
  ...
  try {
    ...
    if (isPaginated) {
      return res.status(200).json({
        ...
        data: { page: pageNum, limit: limitNum, totalPages, totalEntries, hasMore: pageNum < totalPages, data: formattedData }
      });
    } else {
      return res.status(200).json({
        success: true,
...
exports.updateLeadStage = async (req, res, next) => {
  try {
    ...
    if (lead.stage === stage) {
      ...
      return res.status(200).json({ success: true, message: 'Lead updated successfully', data: responseLead });
    }
    ...
    if (!allowed.includes(stage)) {
      return res.status(422).json(wrapError('Invalid stage transition'));
    }

    if (stage === 'Won' || stage === 'Lost') {
      return res.status(400).json(wrapError('Cannot close from this endpoint'));
    }
...
exports.closeLeadLost = async (req, res, next) => {
  try {
    ...
    if (stage !== 'Lost') {
      return res.status(400).json(wrapError('Stage must be Lost when closing as lost'));
    }
    ...
    if (!validReasons.includes(lost_reason)) {
      return res.status(400).json(wrapError('Invalid lost reason'));
    }
    ...
    const responseLead = { ...updatedLead, status: updatedLead.lead_status };
    return res.status(200).json({ success: true, message: 'Lead closed as Lost', data: responseLead });
  } catch (error) {
    next(error);
  }
};
...
exports.closeLeadWon = async (req, res, next) => {
  try {
    ...
    if (stage !== 'Won') {
      return res.status(400).json(wrapError('Stage must be Won when closing as won'));
    }
    ...
    if (isNaN(numericValue)) {
      return res.status(400).json(wrapError('final_deal_value must be a number'));
    }
    if (numericValue < 0) {
      return res.status(400).json(wrapError('final_deal_value must be a non-negative number'));
    }
    ...
    if (closureDateOnly < createdDateOnly) {
      return res.status(400).json(wrapError('closure_date cannot be before lead creation date'));
    }
    ...
    if (lead.stage !== 'Negotiation') {
      return res.status(422).json(wrapError('Cannot close as Won. Lead must be in Negotiation stage.'));
    }
    ...
    const responseLead = { ...updatedLead, status: updatedLead.lead_status };
    return res.status(200).json({ success: true, message: 'Lead closed as Won', data: responseLead });
  } catch (error) {
    next(error);
  }
};
