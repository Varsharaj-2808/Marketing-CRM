...
describe('STORY-2.1.4: Lead Stage Management Tests', () => {
  ...
  describe('API-1: PUT /marketing/leads/:id/status', () => {
    ...
    test('test-ep-2.4.1-008 (Negative): Illegal transition from New Lead directly to Won', async () => {
      ...
      expect((res.body.body && res.body.body.error) || res.body.error || res.body.message).toBe("Invalid stage transition from 'New Lead' to 'Won'. Allowed transitions: Contacted, Hold, Lost");
    });
    ...
    test('test-ep-2.4.1-009 (Negative): Illegal transition from New Lead directly to Meeting Scheduled', async () => {
      ...
      const res = await request(createTestApp())
        ...
        .send({ stage: 'Meeting Scheduled' });
      expect(res.status).toBe(422);
      expect((res.body.body && res.body.body.error) || res.body.error || res.body.message).toBe("Invalid stage transition from 'New Lead' to 'Meeting Scheduled'. Allowed transitions: Contacted, Hold, Lost");
    });
    ...
    test('test-ep-2.4.1-010 (Negative): Illegal transition from Contacted directly to Won', async () => {
      ...
      const res = await request(createTestApp())
        ...
        .send({ stage: 'Won' });
      expect(res.status).toBe(422);
      expect((res.body.body && res.body.body.error) || res.body.error || res.body.message).toContain("Allowed transitions");
    });
    ...
    test('test-ep-2.4.1-012 (Negative): ME attempts to update stage on a Won lead', async () => {
      ...
      expect((res.body.body && res.body.body.error) || res.body.error || res.body.message).toBe('This lead is closed. Contact Admin to reopen.');
    });
    ...
    test('test-ep-2.4.1-016 (Negative): Unauthenticated request', async () => {
      ...
      expect((res.body.body && res.body.body.error) || res.body.error || res.body.message).toBe('Authentication required');
    });
    ...
    test('test-ep-2.4.1-052 (Negative): Expired JWT token on stage transition', async () => {
      const res = await request(createTestApp())
        ...
        .send({ stage: 'Contacted' });
      expect(res.status).toBe(401);
      expect((res.body.body && res.body.body.error) || res.body.error || res.body.message).toBe('Invalid or expired token');
    });
  ...
  describe('API-3: PUT /marketing/leads/:id/close', () => {
    ...
    test('test-ep-2.4.1-035 (Negative): Close as Won from stage that is not Negotiation (blocked)', async () => {
      ...
      const res = await request(createTestApp())
        ...
        .send({ stage: 'Won', final_deal_value: 50000, closure_date: '2026-07-15' });
      expect(res.status).toBe(422);
      expect((res.body.body && res.body.body.error) || res.body.error || res.body.message).toBe("Cannot close as Won from stage 'Contacted'. Lead must be in 'Negotiation' stage.");
    });
  ...
  describe('API-4: POST /admin/leads/:id/reopen', () => {
    ...
    test('test-ep-2.4.1-040 (Negative): Marketing Executive attempts to reopen a closed lead', async () => {
      ...
      const res = await request(createTestApp())
        ...
        .send({ reason: 'Client requested re-engagement' });
      expect(res.status).toBe(403);
      expect((res.body.body && res.body.body.error) || res.body.error || res.body.message).toBe('Forbidden. Admin role required.');
    });
    ...
    test('test-ep-2.4.1-041 (Negative): Admin attempts to reopen without providing a reason', async () => {
      ...
      const res = await request(createTestApp())
        ...
        .send({});
      expect(res.status).toBe(400);
      expect(res.body.reason || (res.body.body && res.body.body.errors && res.body.body.errors.reason)).toBe('Reopen reason is required');
    });
    ...
    test('test-ep-2.4.1-042 (Negative): Admin attempts to reopen with empty reason', async () => {
      ...
      const res = await request(createTestApp())
        ...
        .send({ reason: '' });
      expect(res.status).toBe(400);
      expect(res.body.reason || (res.body.body && res.body.body.errors && res.body.body.errors.reason)).toBe('Reopen reason cannot be empty');
    });
    ...
    test('test-ep-2.4.1-059 (Negative): Admin reopen reason exceeds maximum length', async () => {
      ...
      expect(res.status).toBe(400);
      expect(res.body.reason || (res.body.body && res.body.body.errors && res.body.body.errors.reason)).toBe('Reopen reason must not exceed 500 characters');
    });
    ...
    test('test-ep-2.4.1-043 (Negative): Admin attempts to reopen a lead that is not closed (already active)', async () => {
      ...
      const res = await request(createTestApp())
        ...
        .send({ reason: 'Test' });
      expect(res.status).toBe(400);
      expect((res.body.body && res.body.body.error) || res.body.error || res.body.message).toBe('Lead is not closed. Current stage: Contacted');
    });
