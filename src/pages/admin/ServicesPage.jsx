    ...
    <div className="mt-4">
      <div className="flex flex-col sm:flex-row items-start sm:items-end justify-between gap-3 mb-6">
        <div>
          ...
          <h1 className="font-headline-lg text-on-surface">Services</h1>
          <p className="font-body-md text-on-surface-variant mt-1">Manage service offerings available for lead selection.</p>
        </div>
        <button
          onClick={openCreate}
      ...
      <div className="glass-card overflow-hidden mb-6">
        <div className="p-5 border-b border-outline-variant/10">
          <h4 className="font-headline-md text-headline-md text-on-surface">All Services</h4>
        </div>
              ...
              {services.length === 0 ? (
                <tr>
                  <td colSpan={3} className="py-12 text-center text-on-surface-variant">
                    <span className="material-symbols-outlined text-4xl block mb-2 opacity-40">handyman</span>
                    No services found. Click "Add Service" to create one.
                  </td>
                </tr>
              ) : (
                services.map((service, index) => (
                  <tr key={service.id} className="border-b border-outline-variant/10 hover:bg-primary/[0.03] transition-colors">
                    <td className="py-3 px-3 text-on-surface-variant">{index + 1}</td>
                    <td className="py-3 px-3 font-medium text-on-surface">{service.name}</td>
          ...
          <div className="bg-white rounded-3xl shadow-2xl w-full max-w-md mx-4" style={{ animation: 'fade-in-up 0.3s cubic-bezier(0.16, 1, 0.3, 1)' }}>
            ...
            <form onSubmit={(e) => { e.preventDefault(); handleSave(); }} className="p-6 space-y-4">
              <div className="space-y-1">
                <label className="font-label-md text-label-md text-on-surface block">Service Name</label>
                <input
                  value={formName}
