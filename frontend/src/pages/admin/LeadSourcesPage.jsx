    ...
    <div className="mt-4">
      ...
      <div className="flex flex-col md:flex-row md:items-center justify-between gap-4 mb-6">
        <div>
          ...
          <h1 className="text-2xl font-bold tracking-tight text-slate-900">Lead Sources</h1>
          <p className="text-sm text-slate-500 mt-0.5">Manage lead source options used in lead creation forms.</p>
        </div>
        <button
          onClick={openCreate}
      ...
      <div className="bg-white border border-slate-200 rounded-xl shadow-sm overflow-hidden mb-6">
        <div className="px-6 py-4.5 border-b border-slate-100 flex items-center justify-between bg-slate-50/50">
          <h4 className="text-base font-semibold text-slate-900">All Lead Sources</h4>
        </div>
              ...
              {sources.length === 0 ? (
                <tr>
                  <td colSpan={3} className="py-12 text-center text-slate-500 bg-white">
                    <span className="material-symbols-outlined text-4xl block mb-2 opacity-30 text-slate-400">source</span>
                    No lead sources found. Click "Add Source" to create one.
                  </td>
                </tr>
              ) : (
                sources.map((source, index) => (
                  <tr key={source.id} className="border-b border-slate-150 hover:bg-slate-50/50 transition-colors duration-150">
                    <td className="py-4 px-6 text-slate-500 font-medium">{index + 1}</td>
                    <td className="py-4 px-4 font-semibold text-slate-900">{source.name}</td>
          ...
          <div className="bg-white rounded-xl shadow-xl w-full max-w-md mx-4 overflow-hidden border border-slate-200" style={{ animation: 'fade-in-up 0.25s cubic-bezier(0.16, 1, 0.3, 1)' }}>
            ...
            <form onSubmit={(e) => { e.preventDefault(); handleSave(); }} className="p-6 space-y-4">
              <div className="space-y-1.5">
                <label className="text-xs font-bold text-slate-700 uppercase tracking-wide block">Source Name</label>
                <input
                  value={formName}
