...
exports.createSavedView = async (req, res, next) => {
  try {
    ...
    if (typeof name !== 'string' || name.trim().length === 0) {
      return res.status(400).json({ name: 'Name cannot be empty' });
    }
    if (name.length > 100) {
      return res.status(400).json({ name: 'Name must be 100 characters or less' });
    }
...
exports.updateSavedView = async (req, res, next) => {
  try {
    ...
    if (name === undefined && filters === undefined) {
      return res.status(400).json({ error: 'At least one field (name or filters) must be provided' });
    }
    ...
    if (existing.created_by !== req.user.id) {
      return res.status(403).json({ error: 'You do not have permission to modify this saved view' });
    }
...
exports.deleteSavedView = async (req, res, next) => {
  try {
    ...
    if (existing.created_by !== req.user.id) {
      return res.status(403).json({ error: 'You do not have permission to delete this saved view' });
    }
