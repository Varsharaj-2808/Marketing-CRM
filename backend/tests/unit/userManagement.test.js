...
jest.mock('../../src/config/db', () => ({ query: (...args) => mockQuery(...args), getClient: jest.fn().mockResolvedValue({ query: (...args) => mockQuery(...args), release: jest.fn() }) }));
...
describe('2.8 Business Rules ΓÇö USER-059 to USER-066', () => {
  ...
  test('USER-065: Password complexity', () => {
    const { generateTempPassword } = require('../../src/utils/passwordUtils');
    for (let i = 0; i < 10; i++) {
